...
class TrAISformer(nn.Module):
    ...
    def __init__(self, config, roi: ROI, partition_model=None):
        ...
        self.n_embd = self.n_lat_embd + self.n_lon_embd + \
        ...
        self.pos_emb = nn.Parameter(torch.zeros(
            1, config.max_seqlen, self.n_embd))
        self.drop = nn.Dropout(config.embd_pdrop)

        # transformer
        self.blocks = nn.Sequential(*[Block(config, self.n_embd)
        ...
        if self.mode in ("mlp_pos", "mlp"):
            ...
            # Classification head
            self.head = nn.Linear(self.n_embd, self.full_size, bias=False)
    ...
    def configure_optimizers(self, train_config):
        ...
        optimizer = torch.optim.AdamW(optim_groups, lr=train_config.learning_rate, betas=(
...
#             pdb.set_trace()
    ...
    def forward(self, x, masks=None, with_targets=False, return_loss_tuple=False):
        ...
        # (bs, seqlen, full_size) or (bs, seqlen, n_embd)
        logits = self.head(fea)

        lat_logits, lon_logits, sog_logits, cog_logits =\
            torch.split(logits, (self.lat_size, self.lon_size,
                        self.sog_size, self.cog_size), dim=-1)
            ...
            if self.blur:
                ...
                for _ in range(self.blur_n):
                    blurred_lat_probs = self.blur_module(
                        lat_probs.reshape(-1, 1, self.lat_size)).reshape(lat_probs.shape)
                    blurred_lon_probs = self.blur_module(
                        lon_probs.reshape(-1, 1, self.lon_size)).reshape(lon_probs.shape)
                    blurred_sog_probs = self.blur_module(
                        sog_probs.reshape(-1, 1, self.sog_size)).reshape(sog_probs.shape)
                    blurred_cog_probs = self.blur_module(
                        cog_probs.reshape(-1, 1, self.cog_size)).reshape(cog_probs.shape)
